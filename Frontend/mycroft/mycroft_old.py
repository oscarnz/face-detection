...
from components.server import load_video, process_video
import ast
import cv2
from threading import Lock, Thread
from components.url_endpoint import url, endpoint_facemask_yolov5
...
async def mycroft():
    ...
    while True:
        ...
        cv2.rectangle(frame,
                      ...
                      (int(width/3*2), int(height/3*2)), (255, 0, 0), 3)
        wearing_mask_count = 0
        no_mask_count = 0
        if run:
            detecting_frame = frame[int(height/3):int(height/3*2),
                                    int(width/3):int(width/3*2)]
            ...
            for index in range(len(filtered_output)):
                ...
                wearing_mask_count, \
                    ...
                    detecting_frame = draw_frame_and_count(wearing_mask_count,
                                                           ...
                                                           x, y, w, h,
                                                           label)
            frame[int(height/3):int(height/3*2),
                  int(width/3):int(width/3*2)] = detecting_frame
            cv2.rectangle(frame,
                          (int(width/3), int(height/3)),
                          (int(width/3*2), int(height/3*2)), (255, 0, 0), 3)
        with placeholder.container():
